...
def main(ds, model, train_ds):
    ...
    gold_df['narrative'] = gold_df['annotation'].swifter.progress_bar(True).apply(extract_narrative_category) #
    ...
    dataset_df['narrative'] = dataset_df['prediction'].swifter.progress_bar(True).apply(extract_narrative_category) #
    ...
    city_dfs = []
    gold_city_dfs = []
    for city_name, city_df in dataset_df.groupby('city'):
        # breakpoint()
    
        x = city_df.narrative.str.join('|').str.get_dummies().add_prefix(f'mean_pred_{city_name}_')
        city_df = pd.concat([city_df, x], axis=1)
        
        city_df = city_df.drop(['file_id', 'lens', 'title', 'loc', 'year', 'month', 'state', 'id', 'completion', 'scope', 'prediction', 'narrative', 'contains',  'city'], axis=1)
    ...
    dataset_df = dataset_df.groupby(['year_month', 'cand_sample_prob']).mean().reset_index()
