...
class OpenAIAPICache(APICache):
    ...
    def __init__(self, mode: str = "completion", **redis_kwargs: dict):
        ...
        if mode == "completion":
            ...
            self.api_call = self.openai.chat.completions.create
        super().__init__(**redis_kwargs)

class ClaudeAPICache(APICache):
    """A cache wrapper for Claude's Chat and Completion API calls.
    ...
    def __init__(self, client: anthropic.Anthropic, **redis_kwargs: dict):
        """Initializes an OpenAIAPICache Object.

        Args:
            port: Port of the Redis backend.
            mode: "completion" or "messages", determines which API to call
        """
