...
PROMPT_DICT_BINARY = {
    "prompt": Path("/net/scratch/mourad/economic-narratives/src/prompt-templates/sft/phi2/prompt_binary.txt").read_text(),
    # "prompt_infer": 
...
def preprocess(instance, tokenizer):
    ...
    for idx in range(batch_size):
        instr = PROMPT_DICT_BINARY['prompt'].format(SENTENCE=instance['text'][idx])
        template = instance['template'][idx].split("#")
        data = instance['data'][idx].split("#")
        input = tokenizer(instr)['input_ids']
        label = [-100] * len(input)
        answer = "Yes" if data[1] == 'true' else "No"
        dtoks = tokenizer(answer)['input_ids']
        input += dtoks
        label += dtoks
        print(answer)
