...
def flatten_gold_json(id, data):
    def extract_narrative_info(row):
        ...
        return {"cause_or_effect": cause_effect, "category": category, "time": category_time}

    general_df = pd.DataFrame(data, index=[0])
    general_df = general_df.drop('inflation-narratives', axis=1)
    general_df['id']= id
    narrative_df = pd.DataFrame(data['inflation-narratives'])
    narrative_df['id'] = id
    ...
    general_df['foreign'] = data['foreign']
    general_df['contains-narrative'] = data['contains-narrative']

    narrative_cols = [ "category", "time", "cause_or_effect"]
    general_cols = ['id', 'foreign', 'contains-narrative', 'inflation-time', 'counter-narrative']

    for col in general_cols:
        if col not in general_df.columns:
            if col in narrative_df:
                general_df[col] = narrative_df[col]
                narrative_df= narrative_df.drop(col, axis=1)
            else:
                general_df[col] = ""         
    if "narratives" not in narrative_df.columns:
        narrative_df['narratives'] = None
    if narrative_df.empty or not data['contains-narrative']:
        ...
        for nc in narrative_cols:
            ...
            # except: breakpoint()
        narrative_df['id'] = id
        narrative_df = narrative_df.drop('narratives', axis=1)
        return general_df, narrative_df

    narr_data = narrative_df['narratives'].apply(extract_narrative_info)

    narr_df = narr_data.apply(pd.Series)
    narrative_df = pd.concat([narrative_df, narr_df], axis=1)
    narrative_df = narrative_df.drop('narratives', axis=1)
    return general_df, narrative_df
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', choices=['claude', 'gpt35', 'gpt4t', "gpt4", "phi2_ft", "phi3_ft", "mistral_ft", "all"], default='gpt4t')
